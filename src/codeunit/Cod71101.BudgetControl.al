codeunit 71101 "Budget Control"
{
    ...
    /*  [EventSubscriber(ObjectType::Codeunit, Codeunit::"Gen. Jnl.-Post", OnBeforeCode, '', false, false)]
      ...
      procedure OnModifyGenJournal(var Rec: Record "Gen. Journal Line")
      begin
      ...
      procedure OnAfterGLBudgetInsert(var Rec: Record "G/L Budget Entry"; RunTrigger: Boolean)
      var
          BudgetNameRec: Record "MASQ Budget";
          GLaccount: Record "G/L Account";
          Job: record job;
      begin
          Clear(BudgetNameRec);
          BudgetNameRec.Get(Rec."Budget Name");
          GLaccount.Get(Rec."G/L Account No.");
          Rec."Starting Date" := BudgetNameRec.StartingDate;
          Rec."Ending Date" := BudgetNameRec.EndingDate;
          Rec."G/L Account Type" := GLaccount."Account Type";
          clear(Job);
          job.SetRange("Apollo Project Number", Rec."Global Dimension 1 Code");
          IF Job.FindFirst() then begin
              Rec."Project Number" := Job."No.";
          end else begin
              Rec."Project Number" := '';
          end;
          Rec.Modify(true);

          InsertPostedEntry(Rec, Rec, BudgetNameRec);

      end;

      Local procedure InsertPostedEntry(var Rec: Record "G/L Budget Entry"; var GLBudgetEntry: Record "G/L Budget Entry"; var GLBudgetName: Record "G/L Budget Name")
      var
      ...
      begin
          GLSetup.Get();
          Clear(GLPostedEntry);
          GLPostedEntry."Budget Name" := Rec."Budget Name";
          GLPostedEntry."G/L Account No." := Rec."G/L Account No.";
          GLPostedEntry."G/L Account Type" := Rec."G/L Account Type";
          GLPostedEntry."Global Dimension 1 Code" := Rec."Global Dimension 1 Code";
          GLPostedEntry."Global Dimension 2 Code" := Rec."Global Dimension 2 Code";
          Clear(DimensionSetEntry);
          DimensionSetEntry.SetRange("Dimension Set ID", Rec."Dimension Set ID");
          IF DimensionSetEntry.FindSet() then
              repeat
                  IF DimensionSetEntry."Dimension Code" = GLSetup."Shortcut Dimension 3 Code" then
                      GLPostedEntry."Global Dimension 3 Code" := DimensionSetEntry."Dimension Value Code";
                  IF DimensionSetEntry."Dimension Code" = GLSetup."Shortcut Dimension 4 Code" then
                      GLPostedEntry."Global Dimension 4 Code" := DimensionSetEntry."Dimension Value Code";
                  IF DimensionSetEntry."Dimension Code" = GLSetup."Shortcut Dimension 5 Code" then
                      GLPostedEntry."Global Dimension 5 Code" := DimensionSetEntry."Dimension Value Code";
                  IF DimensionSetEntry."Dimension Code" = GLSetup."Shortcut Dimension 6 Code" then
                      GLPostedEntry."Global Dimension 6 Code" := DimensionSetEntry."Dimension Value Code";
                  IF DimensionSetEntry."Dimension Code" = GLSetup."Shortcut Dimension 7 Code" then
                      GLPostedEntry."Global Dimension 7 Code" := DimensionSetEntry."Dimension Value Code";
                  IF DimensionSetEntry."Dimension Code" = GLSetup."Shortcut Dimension 8 Code" then
                      GLPostedEntry."Global Dimension 8 Code" := DimensionSetEntry."Dimension Value Code";

              until DimensionSetEntry.Next() = 0;
          GLPostedEntry.Date := Rec.Date;
          GLPostedEntry."Starting Date" := Rec."Starting Date";
          GLPostedEntry."Ending Date" := Rec."Ending Date";
          GLPostedEntry."Project Number" := Rec."Project Number";
          GLPostedEntry."Budgeted Amount" := Rec."Amount";
          GLPostedEntry.Description := Rec.Description;
          GLPostedEntry."Business Unit Code" := Rec."Business Unit Code";
          GLPostedEntry."User ID" := Rec."User ID";
          GLPostedEntry."G/L Budget Entry No." := Rec."Entry No.";
          GLPostedEntry.Insert(true);

      end;

      procedure UpdateBudgetValues(GLBudgetName: Code[20])
      var
      begin
    ...
    begin
        ...
        IF BudgetEntry."G/L Account Type" in ["MASQ G/L Account Type"::Total, "MASQ G/L Account Type"::"End-Total"] then
